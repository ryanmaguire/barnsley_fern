...
TOTAL = WIDTH * HEIGHT * 200

# Number of iterations to be run before printing an update to the screen.
UPDATE = 1000000

# Computes a color using a linear HSV color gradient.
def compute_color(val, max_val):
    """
        Function Name:
            compute_color
        Purpose:
            Computes a color using a linear rainbow gradient.
        Arguments:
            val:
                The value for the color.
            max_val:
                The maximum allowed value for a color.
        Outputs:
            color:
                An ordered triple, the color in RGB format.
    """
    if val < 0.0:
        return (0, 0, 255)
    ...
    # Blue green to green.
        ...
        red = 0
        green = 255
        blue = 255 - int(1024.0*scaled_val)

    # Next we green to yellow.
    elif scaled_val < 0.75:
        scaled_val -= 0.5
        blue = 0
        green = 255
        red = int(1024.0*scaled_val)
    ...
    # Finally, yellow to red.
        ...
        scaled_val -= 0.75
        red = 255
        green = 255 - int(1024.0*scaled_val)
        blue = 0
    else:
        return (255, 0, 0)
...
def write_file(data, filename):
    ...
    for y_val in range(HEIGHT):
        for x_val in range(WIDTH):
            ...
            val = grey*grey / (1.0 + grey*grey)
            color = compute_color(val, max_val)
            file.write("%u %u %u\n" % color)
...
if __name__ == "__main__":
    ...
    write_file(pixels, "barnsley_fern_rgb_gradient.ppm")
